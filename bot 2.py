...
async def send_daily_poll():
    ...
    while True:
        ...
        for chat_id in active_chats:
            ...
            try:
                await bot.send_message(chat_id, "🔥 Ежедневный опрос: как с алко сегодня?", reply_markup=keyboard)
...
async def cmd_stats(message: Message):
    ...
    msg = (
        ...
        f"Период: {start_date_str} - {end_date_str}\n"
        f"⏳ Прошло: {days_passed} дней, осталось: {days_left} дней.Держимся!\n\n"
    )
...
async def cmd_report(message: Message):
    ...
    await message.answer("📉 Отметил срыв. Могли бы тебя поддержать, но не будем!")
...
async def cmd_mark_sober(message: Message):
    ...
    await message.answer("🎉 Трезвый день записан! А ты ничего;)")
...
async def handle_poll_response(callback: CallbackQuery):
    ...
    if callback.data == "not_drink":
        ...
        await update_stat(chat_id, user_id, "drinks")
        await callback.answer("📉 Я всё записываю...!")
